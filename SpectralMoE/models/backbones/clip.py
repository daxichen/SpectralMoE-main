...
class CLIPVisionTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # # 创建新权重张量（输出通道，输入通道=4，高，宽）
            # new_conv1 = torch.zeros((C_o, 4, H, W), dtype=conv1.dtype)
            
            # # 通道复制逻辑（新通道i对应原始通道i%3）
            # for i in range(4):
            #     new_conv1[:, i, :, :] = conv1[:, i % 3, :, :]
                
            # # 调整空间尺寸
            # new_conv1 = torch.nn.functional.interpolate(
            #     new_conv1.float(),
